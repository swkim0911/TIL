...
> GC Root는 GC가 객체 탐색을 시작하는 출발점으로 GC Root에서 직·간접적으로 참조된 모든 객체는 GC되지 않는다. 예를 들어 로컬 스택에 저장된 메서드의 지역 변수가 있다.


### Minor GC의 동작 방식
Minor GC의 동작을 이해하기 전에 Young 영역의 구조에 대해 알아야 한다. Young 영역읜 1개의 Eden 영역과 2개의 Survivor 영역으로 구성된다.
- Eden 영역: 새로 생성된 객체가 할당(Aollocation)되는 영역
- Survivor 영역: 최소 1번 이상의 GC로부터 살아남은 객체가 존재하는 영역

객체가 새로 생성되면 Young 영역의 Eden 영역에 할당된다. 그리고 Eden 영역이 꽉 차면 Minor GC가 발생하는데, Mark 되지 않은 객체 메모리는 해제되고 Eden 영역에 남은 객체는 Survivor 영역으로 옮겨진다. 
